...
<div class="p-6 text-center">
    <h1 class="text-3xl font-bold mb-6">Pilih Level Pengurangan</h1>

    <a id="btn-level1" 
       href="{{ route('quiz.level', ['operation'=>'pengurangan','level'=>1]) }}"
       class="block bg-green-600 text-white p-4 rounded-xl mb-4">
       Level 1
    </a>

    <a id="btn-level2" 
       href="{{ route('quiz.level', ['operation'=>'pengurangan','level'=>2]) }}"
       class="block bg-gray-400 text-white p-4 rounded-xl mb-4 pointer-events-none opacity-40">
       Level 2 (Terkunci)
    </a>

    <a id="btn-level3" 
       href="{{ route('quiz.level', ['operation'=>'pengurangan','level'=>3]) }}"
...
<script>
    ...
    const completedLevels = @json(session('pengurangan_completed_levels', []));
