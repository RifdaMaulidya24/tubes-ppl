            ...
            @for($i = 1; $i <= 5; $i++)
                ...
                @if($isUnlocked)
                    <a href="{{ route('quiz.level', ['operation'=>'pengurangan','level'=>$i]) }}"
                       class="group block bg-gradient-to-r {{ $isCompleted ? 'from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700' : 'from-red-500 to-red-600 hover:from-red-600 hover:to-red-700' }} text-white p-6 rounded-xl shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-xl">
