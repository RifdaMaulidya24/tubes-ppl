    ...
    <div class="relative max-w-md mx-auto p-6 space-y-6 animate-slideUp">
        ...
        <div class="space-y-4">
            @for($i = 1; $i <= 5; $i++)
                ...
                @if($isUnlocked)
                    <a href="{{ route('quiz.level', ['operation'=>'penambahan','level'=>$i]) }}"
                       class="group block {{ $isCompleted ? 'bg-white/95 hover:bg-white' : 'bg-white/90 hover:bg-white/95' }} text-blue-600 p-6 rounded-2xl shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-2xl no-underline border-2 {{ $isCompleted ? 'border-blue-400' : 'border-blue-300' }}">
                        <div class="flex items-center justify-between">
                            ...
                            @if($isCompleted)
                                <div class="flex items-center space-x-2">
                                    <span class="text-sm bg-blue-100 text-blue-700 px-3 py-1 rounded-full font-medium">✓ Selesai</span>
                                    @if(isset($scores[$i]))
