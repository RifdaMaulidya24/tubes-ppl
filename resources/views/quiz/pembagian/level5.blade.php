...
<head>
  ...
  <title>Quiz Level 5 - Pembagian</title>
      ...
      <div class="w-full max-w-3xl">
        ...
        <div id="section-tutor" class="transition-all duration-500 ease-out text-center text-white max-w-xl mx-auto px-6 py-10 sm:py-14">
          ...
          <p class="mt-6 text-white/80 text-gl sm:text-[17px] leading-relaxed max-w-md mx-auto animate-fade-zoom delay-200">
            Ini level terakhir! Santai tapi fokus, kamu pasti bisa.
          </p>

          <div class="mt-6 rounded-3xl bg-white/50 border border-white/15 p-4 text-left animate-fade-zoom delay-300">
            <p class="text-sm text-orange-900">Tips: cek ulang dengan <b>perkalian balik</b>.</p>
...
<script>
const questions = [
  { text: "180 ÷ 9 = ?",   answer: 20 },
  { text: "176 ÷ 11 = ?",  answer: 16 },
...
function saveProgress(finalScore) {
  fetch('/quiz/pembagian/complete-level/5', {
    method: 'POST',
    headers: {
...
function unlockAndGoToNextLevel() {
  ...
  fetch('/quiz/pembagian/complete-level/5', {
    ...
    body: JSON.stringify({ level: 5, score: finalScore, next_level: 6, unlock_next: true })
  })
  .then(r => r.json())
  .then(data => {
    if (data.status === 'ok' || data.success) window.location.href = '/quiz/pembagian';
    else {
      alert('Gagal menyelesaikan. Silakan coba lagi.');
