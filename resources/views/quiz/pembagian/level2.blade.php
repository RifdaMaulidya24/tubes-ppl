...
<head>
  ...
  <title>Quiz Level 2 - Pembagian</title>
  ...
  <style>
    ...
    @keyframes fadeZoomIn { from { opacity: 0; transform: translateY(10px) scale(0.92); } to { opacity: 1; transform: translateY(0) scale(1); } }
    @keyframes fadeZoomOut { from { opacity: 1; transform: translateY(0) scale(1); } to { opacity: 0; transform: translateY(-10px) scale(0.92); } }
...
<body>
  <div class="min-h-screen w-full relative overflow-hidden">
    ...
    <div class="absolute inset-0" style="background: linear-gradient(to bottom right, #ffa726, #ff9800, #fb8c00);"></div>
    <div class="absolute top-[-150px] left-[-120px] w-[420px] h-[420px] bg-orange-300 opacity-40 blur-[120px] rounded-full"></div>
    <div class="absolute bottom-[-160px] right-[-120px] w-[380px] h-[380px] bg-orange-200 opacity-30 blur-[140px] rounded-full"></div>
      ...
      <div class="w-full max-w-3xl">
        ...
        <div id="section-tutor" class="transition-all duration-500 ease-out text-center text-white max-w-xl mx-auto px-6 py-10 sm:py-14">
          ...
          <div class="mt-7 flex justify-center animate-fade-zoom delay-100">
            <img src="/img/anu.png" alt="Onboarding Pembagian"
          ...
          <p class="mt-6 text-white/80 text-gl sm:text-[17px] leading-relaxed max-w-md mx-auto animate-fade-zoom delay-200">
            Pembagian artinya membagi angka. Yuk baca singkatnya dulu, lalu mulai kuis
          </p>

          <div class="mt-6 rounded-3xl bg-white/50 border border-white/15 p-4 text-left animate-fade-zoom delay-300">
            <p class="text-sm text-orange-900">Pembagian artinya <b>membagi</b> angka.</p>
            <div class="mt-3 rounded-xl bg-white/45 px-10 py-3">
              <p class="text-xs text-orange-900">Contoh:</p>
              <p class="text-2xl font-extrabold tracking-wider text-orange-900">
                24 ÷ 6 = 4
              </p>
            </div>
          </div>
          ...
          <button type="button" onclick="startQuiz()"
            class="mt-7 inline-flex items-center justify-center px-32 py-3.5 rounded-2xl
                   font-extrabold text-base sm:text-lg bg-white text-orange-700
        ...
        <div id="section-quiz" class="hidden transition-all duration-500 ease-out animate-fade-zoom delay-0">
          <div class="max-w-2xl mx-auto text-center text-white">
            ...
            <div class="w-full h-3 rounded-full bg-white/15 overflow-hidden mb-6 border border-white/20 animate-fade-zoom delay-100">
              <div id="progress-bar" class="h-full w-0 bg-gradient-to-r from-orange-200 to-amber-200 transition-all duration-300"></div>
            </div>
        ...
        <div id="section-notification" class="hidden transition-all duration-500 ease-out text-black animate-fade-zoom delay-0">
          <div class="bg-white/95 p-6 rounded-3xl shadow-2xl border border-white/40 max-w-xl mx-auto text-center">
            ...
            <p class="text-lg mb-6">Kamu perlu mengulang <span id="wrong-count" class="font-extrabold text-orange-700"></span>.</p>
        ...
        <div id="section-result" class="hidden transition-all duration-500 ease-out text-black animate-fade-zoom delay-0">
          <div class="bg-white/95 p-6 rounded-3xl shadow-2xl border border-white/40 max-w-xl mx-auto text-center">
            ...
            <div class="text-xl mb-4">Nilai Kamu: <span id="result-score" class="font-extrabold text-3xl text-orange-700"></span></div>
            ...
            <button onclick="unlockAndGoToNextLevel()" id="btn-next-level"
              class="mt-4 block bg-gradient-to-r from-orange-700 to-amber-700 hover:from-orange-800 hover:to-amber-800
            ...
            <button onclick="unlockAndGoBack()" id="btn-go-back"
              class="mt-3 block bg-gradient-to-r from-orange-700 to-amber-700 hover:from-orange-800 hover:to-amber-800
                     text-white p-4 rounded-2xl font-extrabold w-full text-center transition shadow-xl active:scale-[0.99]">
              Kembali
            </button>
          </div>
...
const questions = [
  ...
  { text: "30 ÷ 5 = ?", answer: 6 },
  { text: "32 ÷ 8 = ?", answer: 4 },
  { text: "Ani memiliki 24 permen. Permen tersebut akan dibagikan sama banyak kepada 6 teman saat acara ulang tahun. Berapa permen yang diterima setiap teman?", answer: 4},
  { text: "Budi membeli 25 kue untuk dibagikan ke 5 meja belajar di kelas. Setiap meja mendapat jumlah kue yang sama. Berapa kue di setiap meja?", answer: 5},
  { text: "Di ruang kelas terdapat 27 pensil. Guru ingin membagikan pensil tersebut secara merata kepada 9 siswa. Berapa pensil yang diterima setiap siswa?", answer: 3}, 
  { text: "Ibu membawa 30 jeruk dari pasar. Jeruk tersebut dimasukkan ke dalam 6 kantong dengan jumlah yang sama. Berapa jeruk di setiap kantong?", answer: 5}
];
...
function loadQuestion() {
  ...
  choices.forEach((choice, i) => {
    ...
    button.className =
      ...
      'bg-white text-orange-700 border border-gray-200 ' +
...
function selectAnswer(selectedAnswer, button) {
  ...
  if (selectedAnswer === q.answer) {
    button.classList.add('bg-[#FDDD9C]','text-gray-900','border-[#FDDD9C]','animate-correct');
    ...
    allButtons.forEach(btn => {
      if (parseInt(btn.innerText) === q.answer) {
        btn.classList.add('bg-[#FDDD9C]','text-gray-900','border-[#FDDD9C]','animate-correct');
      }
    });
...
function showResult() {
  ...
  setTimeout(() => {
    ...
    originalQuestions.forEach((q, originalIndex) => {
      ...
      if (isCorrect && correctFirstAttempt) bgColor = 'bg-orange-100';
      ...
      reviewHTML += `<div class="mb-3 p-3 rounded-2xl ${bgColor} text-left">
        ...
        ${correctFirstAttempt ? '<span class="text-orange-700 font-bold">✓ Benar pertama (+10)</span>' :
...
function saveProgress(finalScore) {
  fetch('/quiz/pembagian/complete-level/2', {
    method: 'POST',
    headers: {
...
function unlockAndGoToNextLevel() {
  ...
  .then(r => r.json())
  .then(data => {
    if (data.status === 'ok' || data.success) window.location.href = '/quiz/pembagian/level3';
    else {
...
function unlockAndGoBack() {
  ...
  .then(r => r.json())
  .then(data => {
    if (data.status === 'ok' || data.success) window.location.href = '/quiz/pembagian';
    else {
