...
<head>
  ...
  <title>Quiz Level 4 - Pembagian</title>
      ...
      <div class="w-full max-w-3xl">
        ...
        <div id="section-tutor" class="transition-all duration-500 ease-out text-center text-white max-w-xl mx-auto px-6 py-10 sm:py-14">
          ...
          <p class="mt-6 text-white/80 text-gl sm:text-[17px] leading-relaxed max-w-md mx-auto animate-fade-zoom delay-200">
            Level makin tinggi! Fokus ya, hasil pembagiannya harus tepat.
          </p>

          <div class="mt-6 rounded-3xl bg-white/50 border border-white/15 p-4 text-left animate-fade-zoom delay-300">
            <p class="text-sm text-orange-900">Tips: cek apakah hasilnya <b>bilangan bulat</b>.</p>
...
const questions = [
  ...
  { text: "110 ÷ 11 = ?", answer: 10 },
  { text: "120 ÷ 8 = ?", answer: 15 },
  { text: "126 ÷ 7 = ?", answer: 18 },
  { text: "132 ÷ 11 = ?", answer: 12 },
  {
    text: "Ani memiliki 30 permen. Ia memberikan 6 permen kepada adiknya. Sisa permen akan dibagikan sama banyak kepada 6 temannya. Berapa permen yang diterima setiap teman?",
    answer: 4
  },
  {
    text: "Budi membawa 24 kue ke sekolah. Sebelum dibagikan, 4 kue dimakan bersama guru. Sisanya dibagikan sama rata ke 5 meja. Berapa kue di setiap meja?",
    answer: 4
  },
  {
    text: "Di kelas terdapat 36 pensil. Sebanyak 6 pensil disimpan di lemari. Sisanya akan dibagikan sama banyak kepada 6 siswa. Berapa pensil yang diterima setiap siswa?",
    answer: 5
  },
  {
    text: "Ibu membeli 40 jeruk. Sebanyak 10 jeruk dipakai untuk jus. Sisanya dimasukkan ke dalam 6 kantong dengan jumlah yang sama. Berapa jeruk di setiap kantong?",
    answer: 5
  }
];
...
function saveProgress(finalScore) {
  fetch('/quiz/pembagian/complete-level/4', {
    method: 'POST',
    headers: {
      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '',
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({ level: 4, score: finalScore, unlock_next: false })
  })
  .then(r => r.json())
...
function unlockAndGoToNextLevel() {
  ...
  button.innerHTML = 'Membuka Level 5...';
  ...
  fetch('/quiz/pembagian/complete-level/4', {
    ...
    body: JSON.stringify({ level: 4, score: finalScore, next_level: 5, unlock_next: true })
  })
  .then(r => r.json())
  .then(data => {
    if (data.status === 'ok' || data.success) window.location.href = '/quiz/pembagian/level5';
    else {
      alert('Gagal membuka level berikutnya. Silakan coba lagi.');
      button.disabled = false;
      button.innerHTML = 'Lanjut ke Level 5';
    }
  })
  .catch(() => {
