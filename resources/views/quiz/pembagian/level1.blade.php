...
<head>
  ...
  <title>Quiz Level 1 - Pembagian</title>
          ...
          <div class="max-w-2xl mx-auto text-center text-white">
            ...
            <div class="soal-card mx-auto rounded-[22px] overflow-hidden shadow-2xl border border-white/10 bg-black/55 backdrop-blur-xl animate-fade-zoom delay-150">
              <div class="p-6 sm:p-8">
                <p id="quiz-question" class="text-2xl sm:text-3xl font-extrabold tracking-normal text-white drop-shadow-lg leading-relaxed"> </p><p id="quiz-question" class="text-6xl sm:text-7xl font-extrabold tracking-widest text-white drop-shadow-lg"></p>
              </div>
            </div>
        ...
        <div id="section-notification" class="hidden transition-all duration-500 ease-out text-black animate-fade-zoom delay-0">
          <div class="bg-white/95 p-6 rounded-3xl shadow-2xl border border-white/40 max-w-xl mx-auto text-center">
            ...
            <p class="text-lg mb-6">
              Kamu perlu mengulang <span id="wrong-count" class="font-extrabold text-orange-700"></span>.
            </p>
        ...
        <div id="section-result" class="hidden transition-all duration-500 ease-out text-black animate-fade-zoom delay-0">
          <div class="bg-white/95 p-6 rounded-3xl shadow-2xl border border-white/40 max-w-xl mx-auto text-center">
            ...
            <div class="text-xl mb-4">
              Nilai Kamu:
              <span id="result-score" class="font-extrabold text-3xl text-orange-700"></span>
            </div>
...
const questions = [
  ...
  { text: "20 ÷ 5 = ?", answer: 4 },
  { text: "21 ÷ 7 = ?", answer: 3 },
  { text: "Ani memiliki 24 permen dan ingin membagikannya sama banyak kepada 6 temannya. Setiap teman mendapat berapa permen?", answer: 4 },
  { text: "Budi membawa 25 kue dan membaginya ke dalam 5 kotak dengan jumlah yang sama. Berapa kue di setiap kotak?", answer: 5 },
  { text: "Di kelas ada 27 pensil yang akan dibagikan sama rata kepada 9 anak. Setiap anak mendapat berapa pensil?", answer: 3 },
  { text: "Ibu memiliki 30 jeruk dan membaginya ke dalam 6 kantong dengan jumlah yang sama. Berapa jeruk di setiap kantong?", answer: 5 }
];
...
function saveProgress(finalScore) {
  fetch('/quiz/pembagian/complete-level/1', {
    method: 'POST',
    headers: {
      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '',
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({ level: 1, score: finalScore, unlock_next: false })
  })
  .then(r => r.json())
...
function unlockAndGoToNextLevel() {
  ...
  button.innerHTML = 'Membuka Level 2...';
  ...
  fetch('/quiz/pembagian/complete-level/1', {
    ...
    body: JSON.stringify({ level: 1, score: finalScore, next_level: 2, unlock_next: true })
  })
  .then(r => r.json())
  .then(data => {
    if (data.status === 'ok' || data.success) window.location.href = '/quiz/pembagian/level2';
    else {
      alert('Gagal membuka level berikutnya. Silakan coba lagi.');
      button.disabled = false;
      button.innerHTML = 'Lanjut ke Level 2';
    }
  })
  .catch(() => {
