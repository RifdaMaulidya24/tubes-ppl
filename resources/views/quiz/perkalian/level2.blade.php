...
<head>
  ...
  <title>Quiz Level 2 - Perkalian</title>
  ...
  <style>
    ...
    .zoom-in{animation:zoomIn .3s ease-out forwards} .zoom-out{animation:zoomOut .3s ease-out forwards}
    .fade-in{animation:fadeIn .3s ease-out forwards} .fade-out{animation:fadeOut .3s ease-out forwards}
    @keyframes fadeZoomIn{from{opacity:0;transform:translateY(10px) scale(.92)}to{opacity:1;transform:translateY(0) scale(1)}}
    @keyframes fadeZoomOut{from{opacity:1;transform:translateY(0) scale(1)}to{opacity:0;transform:translateY(-10px) scale(.92)}}
    .animate-fade-zoom{animation:fadeZoomIn .6s cubic-bezier(.2,.8,.2,1) both}
    .animate-fade-zoom-out{animation:fadeZoomOut .45s ease-in both}
    ...
    .delay-400{animation-delay:400ms} .delay-500{animation-delay:500ms} .delay-700{animation-delay:700ms}
    @keyframes float1{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}}
    @keyframes float2{0%,100%{transform:translateY(0)}50%{transform:translateY(-16px)}}
    @keyframes float3{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}
    .animate-float1{animation:float1 6s infinite} .animate-float2{animation:float2 8s infinite} .animate-float3{animation:float3 7s infinite}
    ...
    .animate-smoke1{animation:smoke1 9s infinite} .animate-smoke2{animation:smoke2 11s infinite} .animate-smoke3{animation:smoke3 10s infinite}
    @keyframes pop{0%{transform:scale(.92)}60%{transform:scale(1.06)}100%{transform:scale(1)}}
    .animate-correct{animation:pop .35s ease-out}
    .animate-wrong{animation:none!important}
    @keyframes symbol1{0%,100%{transform:rotate(0) scale(1)}50%{transform:rotate(180deg) scale(1.1)}}
    @keyframes symbol2{0%,100%{transform:rotate(0) scale(1)}50%{transform:rotate(-180deg) scale(1.1)}}
    @keyframes symbol3{0%,100%{transform:rotate(0) scale(1)}50%{transform:rotate(180deg) scale(1.1)}}
    @keyframes symbol4{0%,100%{transform:rotate(0) scale(1)}50%{transform:rotate(-180deg) scale(1.1)}}
    .animate-symbol1{animation:symbol1 8s infinite} .animate-symbol2{animation:symbol2 10s infinite} .animate-symbol3{animation:symbol3 9s infinite} .animate-symbol4{animation:symbol4 11s infinite}
    ...
    <div id="quiz-container" class="relative min-h-screen flex items-center justify-center px-4 py-10 sm:py-14">
      <div class="w-full max-w-3xl">
        ...
        <div id="section-tutor" class="transition-all duration-500 ease-out text-center text-white max-w-xl mx-auto px-6 py-10 sm:py-14">
          <h1 class="text-2xl sm:text-3xl font-extrabold leading-tight drop-shadow animate-fade-zoom delay-0">Level 2 • Perkalian</h1>

          <div class="mt-7 flex justify-center animate-fade-zoom delay-100">
            <img src="/img/anu.png" alt="Onboarding Perkalian" class="w-[190px] sm:w-[220px] drop-shadow-2xl select-none pointer-events-none" onerror="this.classList.add('hidden');" />
          </div>
          ...
          <div class="mt-6 rounded-3xl bg-white/50 border border-white/15 p-4 text-left animate-fade-zoom delay-300">
            <p class="text-sm text-purple-800">Perkalian artinya <b>mengalikan</b> angka.</p>
            <div class="mt-3 rounded-xl bg-white/45 px-10 py-3">
              <p class="text-xs text-purple-800">Contoh:</p>
              <p class="text-2xl font-extrabold tracking-wider text-purple-800">3 × 5 = 15</p>
            </div>
          </div>

          <button type="button" onclick="startQuiz()"
            class="mt-7 inline-flex items-center justify-center px-32 py-3.5 rounded-2xl font-extrabold text-base sm:text-lg bg-white text-purple-700 hover:bg-white/70 active:scale-[0.99] shadow-xl transition animate-fade-zoom delay-400">
            Mulai
          </button>
        </div>
        ...
        <div id="section-quiz" class="hidden transition-all duration-500 ease-out animate-fade-zoom delay-0">
          <div class="max-w-2xl mx-auto text-center text-white">
            <div class="flex items-center justify-between mb-4 animate-fade-zoom delay-75">
              ...
              <div class="px-3 py-1 rounded-full bg-white/15 backdrop-blur-md border border-white/20 font-extrabold text-sm shadow-lg">🌟 Quiz</div>
            </div>
        ...
        <div id="section-notification" class="hidden transition-all duration-500 ease-out text-black animate-fade-zoom delay-0">
          <div class="bg-white/95 p-6 rounded-3xl shadow-2xl border border-white/40 max-w-xl mx-auto text-center">
            ...
            <p class="text-lg mb-6">Kamu perlu mengulang <span id="wrong-count" class="font-extrabold text-purple-700"></span>.</p>
            <button onclick="startRetry()"
              class="bg-gradient-to-r from-purple-600 to-fuchsia-600 hover:from-purple-700 hover:to-fuchsia-700 text-white w-full p-4 rounded-2xl font-extrabold text-lg transition shadow-xl active:scale-[0.99]">
              Ulang Soalnya
            </button>
          </div>
        </div>
        ...
        <div id="section-result" class="hidden transition-all duration-500 ease-out text-black animate-fade-zoom delay-0">
          <div class="bg-white/95 p-6 rounded-3xl shadow-2xl border border-white/40 max-w-xl mx-auto text-center">
            ...
            <div class="text-xl mb-4">
              Nilai Kamu: <span id="result-score" class="font-extrabold text-3xl text-purple-700"></span>
            </div>
            ...
            <button onclick="unlockAndGoToNextLevel()" id="btn-next-level"
              class="mt-4 block bg-gradient-to-r from-purple-700 to-fuchsia-700 hover:from-purple-800 hover:to-fuchsia-800 text-white p-4 rounded-2xl font-extrabold w-full text-center transition shadow-xl active:scale-[0.99]">
              Lanjut ke Level 3
            </button>

            <button onclick="unlockAndGoBack()" id="btn-go-back"
              class="mt-3 block bg-gradient-to-r from-purple-700 to-fuchsia-700 hover:from-purple-800 hover:to-fuchsia-800 text-white p-4 rounded-2xl font-extrabold w-full text-center transition shadow-xl active:scale-[0.99]">
              Kembali
            </button>
          </div>
        </div>
...
function saveProgress(finalScore) {
  fetch('/quiz/perkalian/complete-level/2', {
    method: 'POST',
    headers: {
      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '',
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({ level: 2, score: finalScore, unlock_next: false })
  })
  .then(r => r.json())
...
function unlockAndGoToNextLevel() {
  ...
  fetch('/quiz/perkalian/complete-level/2', {
    ...
    body: JSON.stringify({ level: 2, score: finalScore, next_level: 3, unlock_next: true })
  })
  .then(r => r.json())
  .then(data => {
    if (data.status === 'ok' || data.success) window.location.href = '/quiz/perkalian/level3';
    else {
      alert('Gagal membuka level berikutnya. Silakan coba lagi.');
      button.disabled = false;
      button.innerHTML = 'Lanjut ke Level 3';
    }
  })
  .catch(() => {
